...
The installation begins with `operator.yaml`, which installs the Red Hat OpenShift AI operator through OLM (Namespace, OperatorGroup, and Subscription). In this example, the operator installs from the stable-3.x update channel as defined in `operator.yaml`.
