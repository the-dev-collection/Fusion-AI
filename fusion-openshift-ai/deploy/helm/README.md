...
```yaml
namespace:
  create: false                    # Namespace usually pre-exists
...
```bash
helm uninstall rhoai-platform -n redhat-ods-operator
```

**Note:** This removes the Helm release but may leave some resources. Clean up manually if needed:
```bash
