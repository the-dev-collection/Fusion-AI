...
**Dashboard**
- Provides the web UI for managing AI/ML projects
- Deployed in `redhat-ods-applications` namespace

**Workbenches**
- Jupyter notebook environments for data science
- User workbenches deployed in `rhods-notebooks` namespace (configurable)

**KServe**
- Model serving infrastructure
- Supports raw deployment mode (no service mesh required)
- Optional NIM (NVIDIA Inference Microservices) integration

**Model Registry**
- Centralized model versioning and metadata management
- Deployed in `rhoai-model-registries` namespace (configurable)
...
- Enables scalable training and inference

**Training Operator**
- Kubernetes-native training jobs
- Supports PyTorch, TensorFlow, XGBoost, etc.

**TrustyAI**
- Responsible AI evaluation and monitoring
...
- Manages feature engineering workflows

**MLflow Operator**
- Experiment tracking and model lifecycle management

**Llama Stack Operator**
- Advanced LLM services and tooling

**Kueue**
- Batch scheduling and queue-based workload orchestration
...
deploy-rhoai:
  ...
  script:
    ...
    - oc apply -f fusion-openshift-ai/deploy/kubernetes/operator.yaml
    - oc wait --for=jsonpath='{.status.phase}'=Succeeded csv -l operators.coreos.com/rhods-operator.redhat-ods-operator -n redhat-ods-operator --timeout=600s
    - oc apply -f fusion-openshift-ai/deploy/kubernetes/dsc.yaml
  only:
    - main
```
