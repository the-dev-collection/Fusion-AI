...
**🚀 Quick Start for Llama Models**: See the [Llama Deployment Guide](LLAMA_DEPLOYMENT_GUIDE.md) for a streamlined workflow to deploy gated models like Meta Llama 3.

By the end of this guide, you will have:
  - A GitOps-managed InferenceService deployment using Red Hat OpenShift GitOps
  - A vLLM runtime serving open-source and gated LLMs (including Meta Llama)
  - HuggingFace authentication support for gated models
...
│   │   └── hf-token-secret.yaml         # HuggingFace token secret template (for gated models)
│   ├── model-serving-application.yaml   # Default model-serving Argo CD Application CR
│   └── llama-model-serving-application.yaml  # Example Llama model Application CR
└── scripts/
    ├── expose-model.sh                  # OpenShift Route creation helper
    └── create-hf-secret.sh              # HuggingFace token secret creation helper
```
...
## Deploying Gated Models (Meta Llama, etc.)

Some models on HuggingFace require authentication and license acceptance before they can be downloaded. This section explains how to deploy gated models like Meta Llama 3 70B.

### Prerequisites for Gated Models

Before deploying a gated model, you must:

1. **Create a HuggingFace Account**
   - Sign up at [https://huggingface.co](https://huggingface.co)

2. **Accept the Model License**
   - Visit the model page (e.g., [Meta Llama 3 70B](https://huggingface.co/meta-llama/Meta-Llama-3-70B))
...
The easiest way to create the HuggingFace token secret is using the provided script:

```bash
# Create the secret in your target namespace
./fusion-model-serving/scripts/create-hf-secret.sh <namespace> <your-hf-token>
...
- Create the `hf-token-secret` secret
...
### Configuring the Deployment for Gated Models

After creating the secret, you need to enable HuggingFace authentication in your deployment:

#### Step 1: Enable HF_TOKEN in InferenceService

Edit [`fusion-model-serving/gitops/models/kserve-model-serving.yaml`](fusion-model-serving/gitops/models/kserve-model-serving.yaml) and uncomment the HF_TOKEN environment variable:

```yaml
env:
- name: MODEL
  value: "ibm-granite/granite-3.2-8b-instruct"
# Uncomment these lines for gated models:
- name: HF_TOKEN
  valueFrom:
    secretKeyRef:
...
#### Step 2: Include Secret in Kustomization

Edit [`fusion-model-serving/gitops/models/kustomization.yaml`](fusion-model-serving/gitops/models/kustomization.yaml) and uncomment the secret resource:
...
resources:
  ...
  - hf-token-secret.yaml  # Uncomment this line
```

#### Step 3: Deploy Using the Llama Example

Use the provided Llama model configuration as a template:

```bash
# Review and customize the Llama configuration
cat fusion-model-serving/gitops/llama-model-serving-application.yaml

# Update the repoURL to point to your forked repository
# Then apply the configuration
oc apply -f fusion-model-serving/gitops/llama-model-serving-application.yaml
```

### Llama Model Configuration Example

The [`llama-model-serving-application.yaml`](fusion-model-serving/gitops/llama-model-serving-application.yaml) provides a complete example for deploying Meta Llama 3 70B:

```yaml
# Key configuration for Llama 3 70B
- op: replace
...
- op: replace
  ...
  value: "4"  # Llama 70B requires multiple GPUs
- op: replace
  path: /spec/predictor/containers/0/resources/limits/memory
  value: "128Gi"  # Significant memory required
```

### Resource Requirements for Llama Models

Different Llama models have varying resource requirements:

| Model | GPUs | Memory | Notes |
|-------|------|--------|-------|
| Llama 3 8B | 1 | 16Gi | Suitable for most use cases |
| Llama 3 70B | 4 | 128Gi | Requires high-end GPU nodes |
| Llama 3.1 405B | 8+ | 512Gi+ | Enterprise-scale deployment |

Adjust the resource requests and limits in your Application manifest based on your cluster capacity.

### Monitoring Gated Model Deployment

Monitor the deployment to ensure the model downloads successfully:

```bash
# Check InferenceService status
oc get inferenceservice -n model-serving

# Watch pod logs for download progress
oc logs -f <predictor-pod-name> -n model-serving

# Check for authentication errors
oc describe pod <predictor-pod-name> -n model-serving
```

Common issues:
- **401 Unauthorized**: Token is invalid or model license not accepted
- **403 Forbidden**: Token lacks required permissions
- **OOMKilled**: Insufficient memory allocated

### Security Best Practices

When working with HuggingFace tokens:

1. **Never commit tokens to Git**
   - The [`hf-token-secret.yaml`](fusion-model-serving/gitops/models/hf-token-secret.yaml) template uses placeholders
   - Always create secrets separately using the script or `oc create secret`

2. **Use Sealed Secrets for Production**
   - Consider using [Sealed Secrets](https://github.com/bitnami-labs/sealed-secrets) or external secret management
   - Integrate with HashiCorp Vault or AWS Secrets Manager

3. **Rotate Tokens Regularly**
   - Generate new tokens periodically
   - Update secrets using: `oc delete secret hf-token-secret -n <namespace>` then recreate

4. **Limit Token Permissions**
   - Use "Read" access only
   - Create separate tokens for different environments

### Switching Between Open-Source and Gated Models

The setup supports both model types in the same cluster:

**For Open-Source Models** (Granite, Mistral, etc.):
- Use the default [`model-serving-application.yaml`](fusion-model-serving/gitops/model-serving-application.yaml)
- No HF_TOKEN required
- Comment out the secret in kustomization.yaml

**For Gated Models** (Llama, etc.):
- Use [`llama-model-serving-application.yaml`](fusion-model-serving/gitops/llama-model-serving-application.yaml)
- Uncomment HF_TOKEN in the InferenceService
- Include hf-token-secret.yaml in kustomization.yaml

You can deploy both types simultaneously in different namespaces, each with their own Application CR.
